  ...
  return (
    <div style={{ fontFamily: 'Georgia, serif', background: '#f8f5f0', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a2e1a 0%, #2d5a2d 100%)', padding: '5rem 2rem', textAlign: 'center', color: 'white' }}>
        <h2 style={{ fontSize: '3rem', fontWeight: 'bold', marginBottom: '1rem', lineHeight: 1.2 }}>Find Your Next Home</h2>
        ...
        <div style={{ display: 'flex', gap: '1rem', maxWidth: '700px', margin: '0 auto', flexWrap: 'wrap', justifyContent: 'center' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by location or title..." style={{ flex: 2, padding: '0.875rem 1rem', borderRadius: '8px', border: 'none', fontSize: '1rem', minWidth: '200px' }} />
          <input value={maxPrice} onChange={e => setMaxPrice(e.target.value)} placeholder="Max price" type="number" style={{ flex: 1, padding: '0.875rem 1rem', borderRadius: '8px', border: 'none', fontSize: '1rem', minWidth: '120px' }} />
          <select value={bedrooms} onChange={e => setBedrooms(e.target.value)} style={{ flex: 1, padding: '0.875rem 1rem', borderRadius: '8px', border: 'none', fontSize: '1rem', minWidth: '120px' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 2rem' }}>
        <h3 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#1a2e1a', marginBottom: '1.5rem' }}>
          {filtered.length > 0 ? `${filtered.length} Available Properties` : 'No listings yet'}
        </h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
          {filtered.map(listing => (
            <div key={listing.id} onClick={() => router.push(`/listings/${listing.id}`)} style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', cursor: 'pointer' }}
        ...
        <div style={{ marginTop: '4rem', background: 'linear-gradient(135deg, #1a2e1a 0%, #2d5a2d 100%)', borderRadius: '16px', padding: '3rem', textAlign: 'center', color: 'white' }}>
          <h3 style={{ fontSize: '1.75rem', fontWeight: 'bold', marginBottom: '1rem' }}>Are you a landlord?</h3>
